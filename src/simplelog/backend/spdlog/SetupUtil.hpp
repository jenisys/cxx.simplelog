 ...
 **/
inline void setLevel(const Level &value) { ::spdlog::set_level(value); }
...
{
    const auto &assignThisLevel = [=](LoggerPtr log) { log->set_level(level); };
    applyToAny(assignThisLevel, predicate);
}

/**
 * Ensures that all loggers that use at least the new minimal log-level
 *(min_level).
...
inline void setMinLevel(const Level &minLevel)
{
    ...
    const auto assignMinLevel = [=](LoggerPtr log) {
        // SIMPLELOG_DIAG_TRACE("setMinLevel: log={0} from {1} to {2}",
        // log->name(), log->level(), minLevel);
 ...
 * @note Logging registry multi-threading protection mechanism is no longer
 *available
