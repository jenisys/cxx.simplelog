...
inline void inheritSinksFromOther(LoggerPtr& log, const LoggerPtr& other)
{
    ...
    SIMPLELOG_DIAG_TRACE("inheritSinksFromOther: {0} inherits {2} sink(s) from {1}",
                         ...
                         (other->name().empty() ? std::string("DEFAULT_LOGGER") : other->name()), defaultSinks.size());
}
 ...
 **/
inline auto makeLogger(std::string name) -> LoggerPtr { return std::make_shared<::spdlog::logger>(name); }
...
inline auto useOrCreateLogger(const std::string& name) -> LoggerPtr
{
    ...
    if (!logPtr) {
        ...
        if (prototype) {
            ...
            SIMPLELOG_DIAG_TRACE("useOrCreateLogger: Create log={0}  with config from DEFAULT_LOGGER (cloned)",
