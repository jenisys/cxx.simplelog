...
// ----------------------------------------------------------------------------
#define SIMPLELOG_DEFINE_MODULE(var_name, name)                                \
    SIMPLELOG_BACKEND_DEFINE_MODULE(var_name, name)
...
//  placeholders; format describes message schema.
#    define SLOG_FATAL_IF(condition, ...)                                      \
        SIMPLELOG_FATAL_IF(condition, __VA_ARGS__)
#    define SLOG_CRITICAL_IF(condition, ...)                                   \
        SIMPLELOG_CRITICAL_IF(condition, __VA_ARGS__)
#    define SLOG_ERROR_IF(condition, ...)                                      \
        SIMPLELOG_ERROR_IF(condition, __VA_ARGS__)
#    define SLOG_WARN_IF(condition, ...)                                       \
        SIMPLELOG_WARN_IF(condition, __VA_ARGS__)
#    define SLOG_INFO_IF(condition, ...)                                       \
        SIMPLELOG_INFO_IF(condition, __VA_ARGS__)
#    define SLOG_DEBUG_IF(condition, ...)                                      \
        SIMPLELOG_DEBUG_IF(condition, __VA_ARGS__)
...
//  placeholders; format describes message schema.
#    define SLOGM_FATAL_IF(condition, logger, ...)                             \
        SIMPLELOGM_FATAL_IF(condition, logger, __VA_ARGS__)
#    define SLOGM_CRITICAL_IF(condition, logger, ...)                          \
        SIMPLELOGM_CRITICAL_IF(condition, logger, __VA_ARGS__)
#    define SLOGM_ERROR_IF(condition, logger, ...)                             \
        SIMPLELOGM_ERROR_IF(condition, logger, __VA_ARGS__)
#    define SLOGM_WARN_IF(condition, logger, ...)                              \
        SIMPLELOGM_WARN_IF(condition, logger, __VA_ARGS__)
#    define SLOGM_INFO_IF(condition, logger, ...)                              \
        SIMPLELOGM_INFO_IF(condition, logger, __VA_ARGS__)
#    define SLOGM_DEBUG_IF(condition, logger, ...)                             \
        SIMPLELOGM_DEBUG_IF(condition, logger, __VA_ARGS__)
#endif
