...
void assert_loggerHasSameSink(LoggerPtr const& logger, SinkPtr const& sink)
{
...
void assert_loggerHasSameSinks(LoggerPtr const& logger, const Sinks& sinks)
{
    CHECK_NE(logger, nullptr);
...
TEST_CASE("setLevelToAny: Should assigns level if predicate is true")
{
    ...
    auto hasSameName = [](LoggerPtr const& log) { return log->name() == "foo"; };
...
TEST_CASE("setLevelToAny: Should keep level if predicate is false")
{
    ...
    auto hasOthergName = [](LoggerPtr const& log) { return log->name() == "other"; };
...
TEST_CASE("assignSinkToAny: Should assign sink to any matching loggers")
{
    ...
    // -- ACT:
    const auto& hasLoggerSameName = [](LoggerPtr const& log) { return log->name() == "foo_2"; };
